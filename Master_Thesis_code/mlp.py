...
nets = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(5, 2), random_state=1) # initialize

nets.fit(X_train1, ravel(y_train1)) # fit the data to the algorithm

predictions_nets = nets.predict(X_test1)
print (predictions_nets)
#print ("Score:", model.score(X_test, y_test))
score_nets = accuracy_score(y_test1, predictions_nets)
print (score_nets)

score_nets_cv = cross_val_score(nets, X_train1, ravel(y_train1), cv=50)

print ("Accuracy_nets_cv: %0.2f (+/- %0.2f)" % (score_nets_cv.mean(), score_nets_cv.std() * 2))

accuracy_nets_cv = score_nets_cv.mean()