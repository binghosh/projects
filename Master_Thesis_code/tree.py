...
clf.fit(X_train1, ravel(y_train1)) # fit the data to the algorithm

predictions_tree = clf.predict(X_test1)
print (predictions_tree)
#print ("Score:", model.score(X_test, y_test))
score_tree = accuracy_score(y_test1, predictions_tree)
print (score_tree)

score_tree_cv = cross_val_score(clf, X_train1, ravel(y_train1), cv=50)

print ("Accuracy_tree_cv: %0.2f (+/- %0.2f)" % (score_tree_cv.mean(), score_tree_cv.std() * 2))

accuracy_tree_cv = score_tree_cv.mean()